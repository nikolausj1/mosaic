...
**Preferred shape (Justin, 2026-07-28, and it is the better idea):** there is NO starting template. The four square thumbnails fly in from the picker grid, get scanned, and morph directly into the finished layout - raw materials becoming a composition.
**Where the honesty line actually sits (revised):** the original entry demanded the "before" be the template the app would genuinely have used without face-awareness. That was drawn in the wrong place. The thing that would be dishonest is an implied COMPARATIVE claim - showing a bad layout presented as the outcome you would otherwise have gotten. Thumbnails assembling assert no counterfactual at all, so there is nothing to misrepresent; it is honest by construction rather than by discipline. What must stay real: **the final layout is genuinely the one the algorithm chose, and the boxes are genuinely what Vision found.** Separately, do not build a before/after side-by-side in MARKETING framed as "what you would get without the AI" - that is where a real claim would start.
**Three reasons the raw-materials version is also better design:**
1. It is a TRANSITION, not an interruption - it replaces the abrupt picker-to-editor cut instead of bolting a ceremony onto arrival. Most of the "never gate the result" worry below softens accordingly.
2. It covers real work. The Vision pass and layout solve genuinely take time that is currently hidden behind a cut, so the animation costs nothing in perceived speed and may read as faster.
3. It animates the user's actual mental model ("these four become this"), and the picker already knows each thumbnail's frame, so the morph has real geometry to work from.
**Practical snags to plan for:** picker thumbnails are SQUARE crops while final cells are varied aspects with their own crops, so the morph must animate the crop or the photo jumps at the end; full-res photos load asynchronously, so the proxies must be in hand or cells pop mid-flight; and the scanning beat should be tied to the real work where possible and capped where not - a re-enactment is fine, a pure 1.5s hold while the result sits ready is added wait.
